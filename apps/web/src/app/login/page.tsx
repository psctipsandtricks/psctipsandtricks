...
function LoginFormContent() {
  ...
  const isAdminLogin = redirectTarget.startsWith('/admin');
    ...
    <div className="max-w-md mx-auto py-12 px-4 relative">
      ...
      <Card className="p-8 space-y-6 glass-panel border-slate-200/80 dark:border-slate-800/80 shadow-2xl relative z-10">
        <div className="text-center space-y-2">
          ...
          <CardDescription>
            {isAdminLogin
              ? 'Log in with your administrator or staff credentials.'
              : 'Log in to access your student community, mock tests & e-books.'}
          </CardDescription>
        </div>

        {/* Protected Feature Redirect Alert (for non-admin pages) */}
        {isProtectedRedirect && !isAdminLogin && (
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-4 pt-1" noValidate>
          <Input
            ...
            name="email"
            type="email"
            placeholder={isAdminLogin ? 'psctipsandtricksapp@gmail.com' : 'aspirant@psctips.com'}
          ...
          >
            {isAdminLogin ? 'Log In to Admin Panel 👑' : 'Log In with Email'}
          </Button>
        </form>
