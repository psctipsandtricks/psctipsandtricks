...
export default function GroupDiscussionPage() {
  ...
  useEffect(() => {
    if (mounted && !authLoading && !user) {
      router.replace(`/login?redirect=/community/${groupIdFromUrl}`);
    }
  }, [mounted, user, authLoading, router, groupIdFromUrl]);
          ...
          <div className="flex items-center justify-between gap-1">
            <div className="flex items-center space-x-1 min-w-0">
              ...
              <h3 className={`text-xs font-extrabold truncate transition-colors ${
                isSelected ? 'text-amber-500' : 'text-slate-900 dark:text-slate-100'
              }`}>
                {group.name}
              </h3>
            </div>
