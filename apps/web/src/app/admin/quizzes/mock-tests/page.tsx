...
const mockTestSchema = Yup.object({
  ...
  scheduledAt: Yup.string()
    ...
    .test('is-future', 'Scheduled time must be in the future', (value) => !!value && new Date(value).getTime() > Date.now()),
});
...
export default function AdminMockTestsPage() {
  ...
  const formik = useFormik({
    initialValues: { title: '', quizId: '', scheduledAt: '' },
    validationSchema: mockTestSchema,
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        if (editingMockTest) {
          await ApiClient.updateMockTest(editingMockTest.id, values);
        } else {
          await ApiClient.createMockTest(values);
        }
  ...
  const handleEdit = (mockTest: any) => {
    ...
    formik.setValues({
      ...
      scheduledAt: mockTest.scheduledAt ? new Date(mockTest.scheduledAt).toISOString() : '',
    });
    setIsDialogOpen(true);
  };
  ...
  const filteredMockTests = mockTests.filter(
    (mt) => statusFilter === 'ALL' || mt.status === statusFilter,
  );
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-slate-200 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-950/70">
                <TableHead className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Title</TableHead>
                <TableHead className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Quiz</TableHead>
                <TableHead className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Scheduled At</TableHead>
                <TableHead className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Status</TableHead>
                <TableHead className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Participants</TableHead>
                <TableHead className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider text-right">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                paginatedMockTests.map((mt) => {
                  ...
                  return (
                    <TableRow key={mt.id} className="border-b border-slate-200/80 dark:border-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      <TableCell className="font-bold text-sm text-slate-900 dark:text-white py-3.5">{mt.title}</TableCell>
                      <TableCell className="text-xs text-slate-600 dark:text-slate-400 py-3.5">{mt.quiz?.title || '—'}</TableCell>
                      <TableCell className="text-xs font-mono text-slate-700 dark:text-slate-300 py-3.5">
      ...
      >
        <form className="space-y-4 pt-2" onSubmit={formik.handleSubmit} noValidate>
          ...
          <DateTimePicker
            ...
            onChange={(v) => {
              formik.setFieldValue('scheduledAt', v);
              formik.setFieldTouched('scheduledAt', true);
            }}
            minDate={new Date()}
            error={formik.touched.scheduledAt && formik.errors.scheduledAt ? formik.errors.scheduledAt : undefined}
          />
